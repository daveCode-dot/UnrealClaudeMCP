...
| **Tools** | **104 live** — 71 native C++ handlers (one MCP method per `Handler_*.cpp`) plus 33 bridge-side synthetic tools (Python-only composition over existing handlers; never crosses the TCP wire as a dedicated round-trip). See [`docs/TOOLS.md`](docs/TOOLS.md) for the per-tool reference. |
...
| **Bridge tests** | 458 pytest cases, ~99% coverage |
