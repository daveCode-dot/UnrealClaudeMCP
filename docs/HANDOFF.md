...
- **Plugin binaries are NOT the live-UE blocker** (correction to my earlier inference). A follow-up Build.bat invocation confirmed `Target is up to date` in 0.86s -- the plugin .dll/.pdb at `F:/ax plug in/HDMediaVirtualStudio/Plugins/UnrealClaudeMCP/Binaries/Win64/` (dated 2026-05-10 23:24) are current against the engine build. The correct Build.bat target is `HDMediaVirtualStudioEditor` (the project's editor target), not `UnrealClaudeMCPEditor` (a non-existent target name). After confirming binaries fresh, a second UE launch attempt with `-unattended -nosplash -stdout` flags still hung in the same idle state: process up, Responding=True, low CPU, port 18888 unbound after 10 minutes, no log writes to `Saved/Logs/HDMediaVirtualStudio.log`. **Root cause is something else** -- candidates: (a) a project-startup modal not suppressed by `-unattended` (perhaps an ESET / SmartScreen security prompt at first launch, a plugin-mismatch warning, or a live-source-control credentials prompt); (b) the project's `Modules: []` in `HDMediaVirtualStudio.uproject` means it's a content-only project, and the plugin loads via `EnabledByDefault: true` in the `.uplugin` rather than the uproject's `Plugins` list -- worth verifying whether content-only projects bind plugin-side TCP servers the same way Source-bearing projects do; (c) something in the project's saved editor state pointing at a missing asset / level / data layer. **Next attended session:** manually launch UE 5.7 against the host project, observe what's on screen at the 30-second / 1-minute / 5-minute marks, and capture which dialog (if any) is blocking. Once identified, that dialog's auto-dismiss is the single remaining piece for fully-autonomous live UE smoke testing.
