...
- **The repo source** — every PR through #65 is on `origin/main` at `github.com/NAJEMWEHBE/UnrealClaudeMCP`
...
- **HANDOFF.md** — reflects the current state (60 tools, 56 C++ handlers + 4 bridge synthetic, all directives, all traps)
...
|---|---|---|
| Repo working tree | `C:\Users\<USERNAME>\Desktop\UnrealClaudeMCP\` | ❌ — re-clone from GitHub |
...
- **60 tools shipped** (56 UE C++ handlers + 4 bridge-side synthetic)
- **Latest commit on main:** `06de2f2` (or whatever's current — `git log -1 --oneline`)
- **Live verification on host machine:** **STILL PENDING** as of the work-PC session pause. This is the highest-value next task post-recovery.
- **Next deferred handler:** `inspect_widget_blueprint` (sibling to `inspect_blueprint`, completes the BP family) — see HANDOFF.md "Autonomy roadmap" for the full list.
...
- **Memory files don't load** — Claude Code reads them via the `MEMORY.md` index. Make sure the destination path matches the project: `~/.claude/projects/C--Users-<USERNAME>-Desktop-UnrealClaudeMCP/memory/`. The folder name encodes the absolute repo path with `\` → `-`.
