...
5. After approval, all 74 tools are available to Claude in chat as `mcp__unreal-claude-mcp__*` (64 dispatched directly to UE, plus 10 bridge-side synthetic tools — camera read/write, focused-actor screenshot, `wait_for_events`, `compile_mod_pak` headless `.pak` build, `bulk_delete_assets`, `inspect_data_asset`, `inspect_sound_class`, `inspect_sound_submix`, and `inspect_audio_bus`)
