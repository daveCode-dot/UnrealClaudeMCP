...
UE 5.7 plugin + Python bridge exposing editor automation to MCP-compliant clients (Claude Code, Codex CLI, Cursor, Gemini CLI, Continue, …) over a localhost TCP socket. **60 tools total: 56 native C++ handlers + 4 bridge-side synthetic tools.** Vendor-neutral — the wire protocol is open MCP; the "Claude" in the repo name is decorative.

## Where to look first for any change

- **C++ handlers** (56) — `UnrealClaudeMCP/Source/UnrealClaudeMCP/Private/MCP/Handlers/Handler_*.cpp`, one per tool. Registered in `UnrealClaudeMCPModule.cpp` (the `Reg.Register(...)` block).
- **Bridge-side synthetic tools** (4: `wait_for_events`, `get_camera_transform`, `set_camera_transform`, `screenshot_actor`) — `bridge/unreal_claude_mcp_bridge.py`'s `SYNTHETIC_TOOLS` dict.
